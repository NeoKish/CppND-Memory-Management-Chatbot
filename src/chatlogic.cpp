...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        {
                            // get iterator on incoming and outgoing node via ID search. Lambda function is being used with std::find_if
                            ...
                            // create new edge
                            // creating instance of GraphEdge to for exclusive ownership of
                            // outgoing GraphEdges
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id);
                           
                            // this involves first derefencing the iterator and then
                            // calling .get() to access the stored GraphNode ptr
    ...
    // create local stack allocated instance of chatbot and 
    // move semantics is used to pass the ChatBot instance to the rootmode
    // ChatLogic would have no ownership of the ChatBot instance and 
    // mo longer responsible for the memory allocation and deallocation
    ChatBot _schatBot("../images/chatbot.png");
    _schatBot.SetChatLogicHandle(this);
    _schatBot.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(_schatBot));
